...
#
# deploy_script.sh
# Chris Vidler - Dynatrace DCRUM SME - 2018
# Deploy and execute a custom script to remote AMDs
#
...
DEPEXEC=1
#default don't restart rtm daemon
RESTART=0
...
HITS=0
OPTS=0
while getopts ":hdrRf:a:u:p:i:x:z:s:" OPT; do
	case $OPT in
		h)
			OPTS=0	#show help
			;;
		...
		s)
			if [ -r $OPTARG ]; then
				...
				OPTS=0
				fatalecho "Deployment script [$OPTARG] not present or inaccessible."
			fi
			;;
		x)
			OPTS=1
...
if [ $OPTS -eq 0 ]; then
	echo -e "*** INFO: Usage: $0 [-h] [-R|r] [-z -z] -a amdaddress|listfile -s scriptfile [-u user] [-p password | -i identfile]"
	echo -e "-h 	This help"
	...
	echo -e "-r 	Restart rtm process once copied."
	echo -e "-R 	DO NOT Restart rtm process once copied. Default."
	echo -e ""
	echo -e "-s scriptfile "
	echo -e "       Custom script to deploy and run on each AMD. Required."
	echo -e ""
...
while read line; do
	...
	AMDADDR=$line
	techo "\e[34m$0\e[0m Deploying ${DEPSCRIPT##*/} to ${AMDADDR}"

	#build SCP command line
	SCPCOMMAND="${DEPPASS}${SCP}${VERBOSE} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p${IDENT} $DEPSCRIPT ${DEPUSER}@${AMDADDR}:${DEPPATH}"
	...
	techo "\e[32m*** SUCCESS:\e[0m Copied [${DEPSCRIPT##*/}] to [${AMDADDR}]."
	if [ $DEPEXEC == 0 ]; then SUCCESS="${SUCCESS}${AMDADDR}\n"; fi

	#build SSH command line to run copied file
	if [ "$DEPEXEC" == "1" ]; then
		if [ "$UNDEPLOY" == "1" ]; then
			SSHCOMMAND="${DEPPASS}${SSH} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${VERBOSE} -tt${IDENT} ${DEPUSER}@${AMDADDR} ${DEPPATH}/${DEPSCRIPT##*/} ${DBG} -z ${RESTART}"
		else
			SSHCOMMAND="${DEPPASS}${SSH} -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${VERBOSE} -tt${IDENT} ${DEPUSER}@${AMDADDR} ${DEPPATH}/${DEPSCRIPT##*/} ${DBG} ${RESTART}"
		fi
		...
		techo "\e[32m*** SUCCESS:\e[0m ${STS} ${DEPSCRIPT##*/} on ${AMDADDR}."
